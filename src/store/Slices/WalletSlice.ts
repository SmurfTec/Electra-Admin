...
export const getPayments = async ({ limit = 5, starting_after = '' }: any) => {
  let params =
    starting_after?.length > 0
      ? `/wallets/payments/?limit=${limit}&starting_after=${starting_after}`
      : `/wallets/payments/?limit=${limit}`;
  try {
    let response: any = await url.get(params);
    return response.data;
  } catch (e) {}
...
export const getTransfers = async ({ limit = 5, starting_after = '' }: any) => {
  let params =
    starting_after?.length > 0
      ? `/wallets/transfers/?limit=${limit}&starting_after=${starting_after}`
      : `/wallets/transfers/?limit=${limit}`;
  try {
    let response: any = await url.get(`${params}`);
    return response.data;
  } catch (e) {}
