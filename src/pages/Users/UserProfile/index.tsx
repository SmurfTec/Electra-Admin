...
import { getSingleUser,GetAllUserOrder,GetUserAsks,GetUserStats,getSingleUserOrder } from '../../../store/Slices/UserSlice'
...
export const UserProfile = () => {
  ...
  const MenuBodyTemplate = (rowData: any) => {
    const MenuTemplate = ({ id, menuRef }: { id: string, menuRef: React.RefObject<any> }) => {
      let [items] = useState([
        {
          ...
          template: (item: any) => {
            ...
            return (
              <div onClick={(event) => viewItem(event, item)} style={{ backgroundColor: 'rgba(255, 245, 0, 0.05)' }} className="flex gap-1 items-center  text-[10px] font-[400] text-[#21212]">
                <SVGIcon
                  fillcolor={'#212121'}
                  src={IMAGES.Ban}
                /> View Item
      ...
      return (
        <CustomMenu model={items} popup ref={menuRef} id="popup_menu_left" popupAlignment="left" height={'80px'} />
      );
    };
